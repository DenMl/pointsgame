...
namespace PointsLibrary
{
	public sealed class Field
	{
		...
		private const Dot EnableMask = Dot.Surrouded | Dot.Putted | Dot.Player;

		private const Dot BoundMask = Dot.Bound | Dot.Surrouded | Dot.Putted | Dot.Player;

		internal const Dot AllowingMask = Dot.Putted | Dot.Surrouded | Dot.BadValue;

		internal const Dot SurroundCountMask = (Dot)0x0000FF00;
		...
		/// <param name="enableCond"></param>
		/// <param name="InputChainDots_"></param>
		/// <param name="InputSurroundedDots_"></param>
		/// <returns></returns>
		private int GetInputDots(int centerPos)
		{
			...
			Dot enableCond = Dots_[centerPos] & EnableMask;
		...
		/// <param name="centerPos"></param>
		/// <param name="enableCond"></param>
		/// <param name="inputChainDots"></param>
		/// <param name="inputSurroundedDots"></param>
		/// <returns></returns>
		...
		{
			if ((Dots_[position] & Dot.Putted) == Dot.Putted)
			{
				if ((Dots_[position] & Dot.Player) != Player)
				{
					LastBaseCaptureCount_++;
					LastMoveCaptureCount_++;
				}
				else
					if ((Dots_[position] & Dot.Surrouded) == Dot.Surrouded)
					{
						LastBaseFreedCount_++;
						LastMoveFreedCount_++;
					}
		...
		{
			if ((Dots_[pos] & Dot.Putted) == Dot.Putted)
				Dots_[pos] |= Dot.Surrouded;
			else
				Dots_[pos] |= Dot.Surrouded | player;

			Dots_[pos] |= (Dots_[pos] & SurroundCountMask) + 0x100;
		}
		...
		private void CheckClosure()
		{
			...
			if ((Dots_[LastPosition_] & Dot.EmptyBase) == Dot.EmptyBase)
			{
				...
				while ((Dots_[chainPosition] & Dot.Putted) != Dot.Putted)
					chainPosition--;
				...
				while ((Dots_[LastPosition_] & Dot.Surrouded) != Dot.Surrouded);
		...
		private void FillBadValues()
		{
			for (int i = 0; i < RealWidth; i++)
				Dots_[i] |= Dot.BadValue;

			for (int i = 1; i < Height + 1; i++)
			{
				Dots_[i * RealWidth] |= Dot.BadValue;
				for (int j = Width + 1; j < RealWidth; j++)
					Dots_[i * RealWidth + j] |= Dot.BadValue;
			}
		...
		{
			return (dot & EnableMask) == enableCondition;
		}

		public static bool IsEnableForEmptyBase(Dot dot, Dot Player)
		{
			return (dot & EnableMask) == (Dot.Putted | Player) ||
				(dot & EnableMask) == (Dot.Putted | Dot.Surrouded | Player.NextPlayer()) ||
				(dot & EnableMask) == (Dot.Surrouded | Player); // Empty position in Player base
		}

		public static bool IsBound(Dot dot, Dot boundCond)
		{
			return (dot & BoundMask) == boundCond;
		}
		...
		public bool UnmakeMove()
		{
			if (DotsSequanceStates_.Count > 0)
			{
				...
				LastMoveState_ = enmMoveState.Removed;
				return true;
			}
			else
			{
